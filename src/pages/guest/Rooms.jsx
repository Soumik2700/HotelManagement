    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-gray-100 font-[Inter]">
      ...
      <div className="bg-gray-900 shadow-sm border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-100">Room Management</h1>
              <p className="text-gray-400 mt-1">Manage hotel rooms and their details</p>
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-gray-900 rounded-xl shadow-lg p-6 flex items-center space-x-4 border border-gray-800">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-100">{roomList.length}</p>
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl shadow-lg p-6 flex items-center space-x-4 border border-gray-800">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-100">{filteredRooms.length}</p> {/* Shows available based on filter */}
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl shadow-lg p-6 flex items-center space-x-4 border border-gray-800">
            ...
            <div>
              <p className="text-sm font-medium text-gray-400">Hotels</p>
              <p className="text-2xl font-bold text-gray-100">{hotels.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl shadow-lg p-6 mb-8 border border-gray-800">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="search" className="block text-sm font-medium text-gray-300 mb-2">Search Rooms</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 border border-gray-700 bg-gray-800 text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200"
                />
              </div>
            </div>

            <div>
              <label htmlFor="filterHotel" className="block text-sm font-medium text-gray-300 mb-2">Filter by Hotel</label>
              <select
                id="filterHotel"
                value={filterHotel}
                onChange={(e) => setFilterHotel(e.target.value)}
                className="w-full px-4 py-3 border border-gray-700 bg-gray-800 text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none pr-8 transition-colors duration-200"
              >
        ...
        {filteredRooms.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredRooms.map((room) => {
              const hotel = hotels.find(h => h.id === room.hotelId);
              return (
                <div key={room.id} className="bg-gray-900 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden border border-gray-800">
                  <div className="relative h-48">
                    ...
                    />
                    <div className="absolute top-2 right-2 bg-blue-700 text-white text-sm font-semibold px-3 py-1 rounded-full shadow-md">
                      ₹{room.pricePerNight?.toLocaleString('en-IN') || 'N/A'} / night
                    </div>
                  </div>
                  <div className="p-6">
                    <h2 className="text-xl font-bold text-gray-100 mb-1">{room.roomType}</h2>
                    ...
                    <div className="mb-4">
                      <h3 className="text-md font-semibold text-gray-300 mb-2">Amenities:</h3>
                      <div className="flex flex-wrap gap-2">
                        {room.amenities && room.amenities.length > 0 ? (
                          room.amenities.map((amenity, index) => (
                            <span key={index} className="bg-blue-900 text-blue-200 text-xs font-medium px-2.5 py-0.5 rounded-full">
                              {amenity}
                            </span>
                          ))
                        ) : (
                    ...
                    <div className="flex justify-end space-x-3 mt-4">
                      <button
                        ...
                        className="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold px-4 py-2 rounded-lg transition-colors duration-200 shadow-md"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDeleteRoomClick(room.id)}
                        className="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded-lg transition-colors duration-200 shadow-md"
                      >
                        Delete
                      </button>
                    </div>
